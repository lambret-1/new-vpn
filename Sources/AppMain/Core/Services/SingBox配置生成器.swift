...
final class SingBox配置生成器 {
    ...
    private func 生成DNS配置(_ DNS配置: DNS配置模型?, 节点: 节点模型?, 运行模式: 隧道运行模式) -> SingBoxDNS配置 {
        // DNS 分流策略：
        // dns_resolver: 阿里云 DNS-over-TLS（tls://223.5.5.5），走 DIRECT，用于国内域名和代理服务器域名解析
        ...
        // dns_proxy: Cloudflare DNS-over-HTTPS（https://1.1.1.1/dns-query），走 proxy，用于国外域名
        //   用 DoH(443端口) 而非 DoT(853端口)：部分代理服务器限制 853 端口转发导致超时
        //   用 1.1.1.1 而非 8.8.8.8：部分代理服务器封锁 Google IP 导致 443 也超时
        let 默认服务器 = [
    ...
    private func 生成路由配置(分流规则: [分流规则项], 节点: 节点模型?, 运行模式: 隧道运行模式) -> SingBox路由配置 {
        ...
        // libbox 版本不支持 TUN 入站 dns_address 字段，必须用路由规则方式拦截
