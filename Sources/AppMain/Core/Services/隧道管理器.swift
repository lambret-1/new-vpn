...
final class 隧道管理器: NSObject, ObservableObject {
    ...
    private func 生成并写入SingBox配置(节点ID: UUID?, 节点名称: String?, 完成: @escaping (Bool) -> Void) {
        ...
        // 生成 sing-box 配置
        // 接入用户在 UI 中配置的分流规则（仅取已启用、按优先级排序）；
        // 规则只在「规则分流」模式下被生成器下发到 route.rules
        let 启用的分流规则 = 分流规则管理器.共享.配置.所有规则
        调试日志管理器.共享.信息("隧道", "载入分流规则：\(启用的分流规则.count) 条（运行模式：\(配置.运行模式.rawValue)）")
    ...
    @objc private func 处理VPN状态变更(_ 通知: Notification) {
        ...
        case .已连接:
            ...
            启动统计定时器()
            // 注意：libbox v1.11.0 不支持 api 入站，网络活动轮询暂不启用
            // 网络活动管理器.共享.开始轮询()
        case .已断开:
